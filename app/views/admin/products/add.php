    ...
    <form action="<?php echo BASE_URL; ?>admin/addProduct" method="post" enctype="multipart/form-data">
        ...
        <div id="simpleProductFields"> <div>
                ...
                <?php foreach($data['all_attributes'] as $attribute): ?>
                    <div style="margin-bottom: 5px;">
                        <input type="checkbox" name="configurable_attributes[]" id="attr_<?php echo $attribute['id']; ?>" value="<?php echo $attribute['id']; ?>"
                               <?php echo (isset($data['configurable_attributes_for_product']) && in_array($attribute['id'], $data['configurable_attributes_for_product'])) ? 'checked' : ''; ?>>
                        <label for="attr_<?php echo $attribute['id']; ?>"><?php echo htmlspecialchars($attribute['name']); ?></label>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <p>ابتدا باید ویژگی‌هایی (مانند رنگ، سایز) را در بخش "مدیریت ویژگی‌ها" تعریف کنید.</p>
            <?php endif; ?>
            <p style="margin-top:15px;"><small>پس از ذخیره این محصول متغیر، به صفحه ویرایش آن بروید تا بتوانید تنوع‌ها (مانند تیشرت قرمز-سایز M) را با قیمت و موجودی مجزا ایجاد کنید.</small></p>
        </div>
        ...
        <fieldset style="margin-top: 20px;">
            ...
            <div>
                ...
                <small>اگر نوع درصدی است، عددی بین ۰ تا ۱۰۰ وارد کنید. اگر مقدار ثابت است، مبلغ را به تومان وارد کنید.</small>
            </div>
        </fieldset>

        <div style="margin-top: 20px;">
            <button type="submit" name="add_product_submit">افزودن محصول</button>
    ...
    <script>
        function toggleProductFields() {
            ...
            if (productType === 'variable') {
                simpleFields.style.display = 'none'; // یا فیلدهای قیمت و موجودی والد را غیرضروری کنید
                variableFields.style.display = 'block';
                priceInput.required = false; // قیمت برای محصول والد متغیر می‌تواند از تنوع‌ها بیاید
                stockInput.required = false; // موجودی محصول والد متغیر می‌تواند مجموع تنوع‌ها باشد یا صفر
